...
SETTING_KEY_FRAMES_TO_SKIP = (
    "frames_to_skip"  # how many frames will be skipped before processing
)
...
class StegoSettings:
    ...
    def serialize(self):
        data = {
            SETTING_KEY_FRAMES_TO_SKIP: self.frames_to_skip,
    ...
    def deserialize(self, filename=SETTINGS_FILE_NAME):
        if os.path.isfile(filename):
            with open(filename, "r") as infile:
                ...
                self.build_in_input_audio_device_name = data[
                    SETTINGS_KEY_BUILD_IN_INPUT
                ]
                self.build_in_output_audio_device_name = data[
                    SETTINGS_KEY_BUILD_IN_OUTPUT
                ]
                self.virtual_audio_device_name = data[SETTINGS_KEY_VIRTUAL_DEVICE]
        else:
            print(
                colorize(
                    f"{SETTINGS_FILE_NAME} couldn't be found. Applying default settings.",
                    COLORS.FAIL,
                )
            )
            self.serialize()
    ...
    def validate_stream_mode(self, stream_mode):
        ...
        if stream_mode == StreamMode.StreamFromFileToFile:
            result = (
                gs_device_info.detect_build_in_input_device_idx(p_audio) >= 0
                and gs_device_info.detect_build_in_output_device_idx(p_audio) >= 0
            )
        elif stream_mode == StreamMode.StreamFromBuildInInputToSoundFlower:
            result = (
                gs_device_info.detect_build_in_input_device_idx(p_audio) >= 0
                and gs_device_info.detect_virtual_audio_device_idx(p_audio) >= 0
            )
